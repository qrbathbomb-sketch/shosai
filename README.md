...
https://qrbathbomb-sketch.github.io/shosai/ (サンプル写真でUI全体を試せる。実アプリはローカル写真を読み取り専用で扱う)
...
- **おまかせセレクト**: 連写検出(EXIF時刻) + 風景色解析(空・緑・彩度) + 顔検出(rustface, オフライン)で景色の良い写真候補を自動選抜
...
npm run dev                                                     # ブラウザ デモモード (http://localhost:1420/)
